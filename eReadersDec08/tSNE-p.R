# Preprocessing of Lambton Library for tSNE - Probabilistic version.
...
workIDs$x <- mxys[,1]
workIDs$y <- mxys[,2]
write.csv(workIDs, "../vis/data/lambton-all.csv", row.names=FALSE)
...
write.csv(
	...
	"../vis/data/lambton-neighbours-all.csv", 
	row.names=FALSE
)

print("Writing out the libraries table...\n")
write.csv(libraries, "../vis/data/libraries.csv", row.names=FALSE)

print("Writing out work/libraries table...\n")
write.csv(libworks, "../vis/data/worklibs.csv", row.names=FALSE)
